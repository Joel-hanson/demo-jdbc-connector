...
case $1 in
    start)
        ...
        if [[ "$CONNECT_CONFIG_STORAGE_TOPIC" = "" ]]; then
        ...
        if [[ "$CONNECT_OFFSET_STORAGE_TOPIC" = "" ]]; then
        ...
        if [[ "$CONNECT_STATUS_STORAGE_TOPIC" = "" ]]; then
